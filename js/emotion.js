$(function () {
    ...
    $items.each(function () {
        ...
        //article에 마우스 클릭했을 때 실행
        $el.on("click", function (e) {
            var $currentTarget = $(e.currentTarget); // 아티클
            var tit = $currentTarget.find("h2").text(); //클릭한 텍스트 h2 가져옴
            var txt = $currentTarget.find("p").text(); //클릭한 텍스트 p 가져옴
            var vidSrc = $currentTarget.find("video").attr("src");
            //클릭한 article 의 비디오 소스 가져옴

            //가져오는 이유는,
            // article의 제목과 내용을 aside의 제목과 내용으로 넣어줌
            ...
            $aside.addClass("on"); //투명도 100%, 위치값 왼쪽 0 의 속성 넣어줌
        });
    });
